...
const HeaderAccount = ({ isAuthenticated, user, color }) => {
    ...
    const { isLoggedout, message } = useSelector((state) => state.user);
    ...
    return (
        <>
            {isLoggedout && <HeaderAlert message={message} />}

            {isAuthenticated ? (
                <NavDropdown
                    id="account-dropdown"
                    title={<span className="font-16" style={{ color: color || '#ffffff' }}>Your account</span>}
                    menuVariant="light"
                ...
                >
                    ...
                    <NavDropdown.Item onClick={() => checkLoginStatus("account")}>
                        Your account
                    </NavDropdown.Item>
                    <NavDropdown.Item onClick={() => checkLoginStatus("orders")}>
                        Your orders
                    </NavDropdown.Item>
                    <NavDropdown.Item onClick={() => checkLoginStatus("logout")}>
                        {user && "Logout"}
                    </NavDropdown.Item>
                </NavDropdown>
            ) : (
                <Stack 
                    ...
                    gap={1} 
                    className="text-decoration-none font-16 px-3 px-md-0 py-2 py-md-0" 
                    as={Link} 
                    to="/login"
                ...
                >
                    <span>Login</span>
                    <img 
                        src={process.env.PUBLIC_URL + "/assets/style/user.svg"} 
                        alt="User icon" 
                        style={{ color: color || '#ffffff' }} 
                    />
                </Stack>
