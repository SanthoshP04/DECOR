...
const OrderDetails = () => {
    ...
    const orderedDate = order?.createdAt ? new Date(order.createdAt).toLocaleDateString() : '';
    ...
    return (
        <>
            ...
            {error && <HeaderAlert error={error} message={message} />}

            <Container className="my-5">
                <Row>
                    <Col lg={8} className="mb-5 pe-md-5">
                        <h1 className="fw-bold font-22 text-blue-500-color mb-2">Order Details</h1>
                        ...
                        <Stack className="bg-gray-300-color px-4 py-5">
                            <h2 className="fw-bold font-18 text-blue-500-color mb-4">Shipping Address</h2>
                            <address>
                                <span className="fw-bold font-16 d-block">{order.user?.name}</span>
                                ...
                                <span className="font-16 d-block">+91 {order.shippingInfo?.phoneNo}</span>
                                <span className="font-16 d-block">{order.user?.email}</span>
                            </address>

                            <h2 className="fw-bold font-18 text-blue-500-color mb-4 mt-5">Payment Details</h2>
                            <span className={order.paymentInfo?.status === "succeeded" ? "text-success mb-5" : "text-danger mb-5"}>
                                {order.paymentInfo?.status === "succeeded" ? "Paid" : "Not paid"}
                            </span>

                            <h2 className="fw-bold font-18 text-blue-500-color mb-4">Order Status</h2>
                            <span className={order.orderStatus === "Delivered" ? "text-success" : "text-danger"}>
                                {order.orderStatus}
                            </span>
                        </Stack>
                    </Col>
