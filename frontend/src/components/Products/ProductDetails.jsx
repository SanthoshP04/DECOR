...
const ProductDetails = () => {
    ...
    const productStock = Array.from({ length: product?.stock || 0 }, (_, index) => index + 1);
    ...
    const submitReviewToggle = () => {
        setOpen(prev => prev === "d-none" ? "d-block" : "d-none");
        setActiveTab('tab2');
    ...
    return (
        <>
            <MetaData title={`${product?.name || 'Product'} -@Hekto`} />
                ...
                <Row className="align-items-center card-shadow p-3">
                    ...
                    <Col sm={12} lg={6}>
                        ...
                        <Stack direction="horizontal" gap={1}>
                            <ReactStars {...options} value={product?.ratings || 0} edit={false} />
                            <span className="text-primary-color">({product?.numberOfReviews || 0})</span>
                        </Stack>
                        <span className="font-26 text-primary-color">₹{product?.price || 0}</span>
                        <p className="mt-2 text-gray-100-color font-16 fw-semibold text-overflow">
                            {product?.description}
                        </p>
                        {product?.stock < 1 ? (
                            ...
                            <>
                                    ...
                                    <Dropdown onSelect={handleStockSelect}>
                                        ...
                                        <Dropdown.Menu className="product-details-dropdown">
                                            {productStock.map((stock, index) => (
                                                <Dropdown.Item key={index} eventKey={stock}>
                                                    {stock}
                                                </Dropdown.Item>
                                            ))}
                                        </Dropdown.Menu>
                                ...
                                <Stack className="flex-column flex-md-row">
                                    <Button 
                                        disabled={product?.stock < 1 || product?.price === 0} 
                                        className="my-2 me-md-2 bg-secondary-color border-0 rounded-0" 
                                        onClick={addProductToCart}
                                    >
                                        Add to Cart
                                    </Button>
                                    <Button 
                                        className="my-2 bg-secondary-color border-0 rounded-0" 
